...
const NavBar = () => {
  ...
  return (
    ...
    <div className='fixed top-0 left-0 w-full flex bg-black/60 h-16 backdrop-blur-md py-3 px-[40px]  justify-between z-10'>
      <div className='hidden p-4 text-black items-center w-1/10 md:flex cursor-pointer'>
        <img
          width='125px'
          height='125px'
      ...
      <div onClick={handleClick} className="flex flex-col gap-2 w-[44px] h-[44px] justify-center md:hidden">
        <motion.div variants={variants} animate={
          isOpen ? 'rotate1': 'base'
        }    className="w-[22px] h-[2px] bg-white"></motion.div>
        <motion.div variants={variants} animate={
          isOpen ? 'middleRotate': 'base'
        } className="w-[18px] h-[2px] bg-white"></motion.div>
        <motion.div variants={variants} animate={
          isOpen ? 'rotate2': 'base'
        } className="w-[22px] h-[2px] bg-white"></motion.div>
      </div>
